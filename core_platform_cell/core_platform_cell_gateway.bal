...
service /coreplatform on corePlatformListener {
    ...
    resource isolated function post register(http:Caller caller, http:Request req) {
        http:Response|http:ClientError response = check serviceRegistrationClient->forward("/service/register", req);
        handleResponse(response, caller);
    }

    // Task management endpoint
    resource isolated function post tasks(http:Caller caller, http:Request req) {
        http:Response|http:ClientError response = taskManagementClient->forward("/task/schedule", req);
        handleResponse(response, caller);
    }

    // Service scaling endpoint
    resource isolated function post scale(http:Caller caller, http:Request req) {
